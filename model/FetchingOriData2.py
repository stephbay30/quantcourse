...
class FetchingOriData:
    ...
    def _load_data(self):
        ...
        for date in pd.date_range(self.start_date, self.end_date):
            pickle_file = f"{self.file_path}/{date.strftime('%Y-%m-%d')}.pkl"
            if os.path.exists(pickle_file):
                with open(pickle_file, 'rb') as handle:
                    daily_data = pickle.load(handle)
                    # 假设Y已经是一个Tensor
                    X_list.append(torch.tensor(daily_data['X'],dtype=torch.float32))
                    Y_list.extend(daily_data['Y'])  # 如果Y是Tensor，转换为numpy数组
                    tokens_list.extend(daily_data['token'])
                    # 为这一天的每个样本添加相同的日期
                    dates_list.extend([date.strftime('%Y-%m-%d')] * len(daily_data['token']))
